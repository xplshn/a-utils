...
build_project() {
    SANDBOX_DEFAULT="--unshare-all --share-net --ro-bind-try /etc/localtime /etc/localtime --ro-bind-try /etc/hostname /etc/hostname --ro-bind-try /etc/resolv.conf /etc/resolv.conf --ro-bind-try /etc/passwd /etc/passwd --ro-bind-try /etc/group /etc/group --ro-bind-try /etc/hosts /etc/hosts --ro-bind-try /etc/nsswitch.conf /etc/nsswitch.conf"
    ...
    unnappear ./noroot-do --set-mode-flags sandbox:"$SANDBOX_DEFAULT" && log "Sandbox flags set"
    unnappear ./noroot-do --sediment sandbox && log 'Sedimented "sandbox" preset'
    log 'Creating "rootsandbox" preset'
    unnappear ./noroot-do --set-mode-flags rootsandbox:"$SANDBOX_DEFAULT $ROOT"
